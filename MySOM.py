...
class Som:
    def __init__(self, dim_x = 2, dim_y = 3, dim_z = 4, input_dim = 3, learn_iter = 10, learning_rate = 0.01, size = 1, sigma = None, time_const = None, quality = 1):
        ...
        quality - denotes how many times horizontal and vertical dimensions of the result will be smaller than in the input image.
        '''
        ...
        if sigma == None:
            self._sigma = max(dim_x, dim_y, dim_z) / 2.0
        else:
            self._sigma = float(sigma)
        if time_const == None:
            self._time_constant = learn_iter / np.log(self._sigma)
        else:
            self._time_constant = time_constant
        self._input_dim = input_dim * (size**2)
        self._learn_iterations = learn_iter
        self._graph = tf.Graph()
        with self._graph.as_default():
            self._weights = tf.Variable(tf.random.normal(shape = [dim_x, dim_y, dim_z, self._input_dim], mean = 400, stddev = 300,  name = "weights"))
            self._input_vec = tf.compat.v1.placeholder("float", [self._input_dim], name = "input_vec")
            self._learning_iteration = tf.compat.v1.placeholder("float", name = "learning_iteration")
            self._input_tensor = tf.stack([tf.stack([tf.stack([self._input_vec for i in range(dim_z)]) for j in range(dim_y)]) for k in range(dim_x)])

            distances_weights = tf.math.sqrt(tf.math.reduce_sum(((self._weights - self._input_tensor)**2.0), axis = 3))
            #self.bmu_distance = tf.math.reduce_min(distances_weights)

            linear_location = tf.math.argmin(
                tf.reshape(
                    tf.reshape(distances_weights, (self._d_x, self._d_y * self._d_z)), (self._d_x * self._d_y * self._d_z, 1)))
            self._bmu_location = [(linear_location // self._d_z // self._d_y) % self._d_x, (linear_location // self._d_z) % self._d_y, linear_location % self._d_z]

            sigma_t = (self._sigma * tf.math.exp(-((self._learning_iteration / self._time_constant))))
            learning_rate_t = (self._lr * tf.math.exp(-((self._learning_iteration / self._time_constant))))
            self._bmu_mask = tf.compat.v1.placeholder("float", shape = [dim_x, dim_y, dim_z])
            self._bmu_gaussian = tf.compat.v1.placeholder("float", shape = [dim_x, dim_y, dim_z])

            #bmu_influence = tf.stack([self._bmu_mask * tf.math.exp(-((self._bmu_gaussian ** 2) / ((sigma_t ** 2) * 2.0))) for i in range(self._input_dim)], axis = 3)
            bmu_influence = tf.stack([self._bmu_mask * self._bmu_gaussian for i in range(self._input_dim)], axis = 3)
            new_weights = self._weights + ((bmu_influence * learning_rate_t) * (self._input_tensor - self._weights))
            ...
            self._saver = tf.compat.v1.train.Saver([self._weights])
            print("SOM created")
    ...
    def train(self, input_vecs):
        ...
        for iter in range(self._learn_iterations):
            for input_vec in input_vecs:
                bmu_loc = self._sess.run(self._bmu_location, feed_dict={self._input_vec: input_vec})
                bmu_mask, bmu_distance = self.__create_mask_for_updating_weights(bmu_loc, iter)
                self._sess.run(self._training_op, feed_dict={self._input_vec: np.reshape(input_vec, (self._input_dim)), self._learning_iteration: float(iter), \
                    self._bmu_mask: bmu_mask, self._bmu_gaussian: bmu_distance})
    def convert(self, input_vec):
        ...
        '''
        return self._sess.run(self._bmu_location, feed_dict={self._input_vec: input_vec})
    def __create_mask_for_updating_weights(self, bmu_location, iter):
        '''
        ...
        '''
        sigma_t = int(self._sigma * np.exp(-((iter / self._time_constant))))
        mask = np.zeros((self._d_x, self._d_y, self._d_z), dtype=np.float32)
        bmu_gaussian = np.zeros((self._d_x, self._d_y, self._d_z), dtype=np.float32)
    ...
    def convert_image(self, img):
        ...
        '''
        converted_img = np.zeros((img.shape[0]//self._quality, img.shape[1]//self._quality, 3))
        for i in self.__generator_image(img.shape[0], img.shape[1]):
            if (i[0]%100 == 0 and i[1] == self._quality):
                print("Converting: " + str(np.floor(100.0 * i[0] / img.shape[0])) + "%")
            input_vec = self.create_input_vec(img, i)
            a = self._sess.run(self._bmu_location, feed_dict={self._input_vec: input_vec}) 
            converted_img[i[0]//self._quality, i[1]//self._quality] = a
        print("converted") 
        return converted_img * (256 // max(self._d_x, self._d_y, self._d_z))
    def __generator_image(self, cols, rows, quality = None):
        '''
    ...
    def train(self, threshold, path):
        ...
        centroids = np.empty((0,self._input_dim))
        while centroids.shape[0] < 30:
            for filename in filenames:
                if filename.find(".lan") > 0:
                    ...
                    continue
                if img.shape[2] != self._input_dim / (self._size**2):
                    continue
                for i in self.__generator_image(img.shape[0], img.shape[1], quality = 1):
                    input_vec = np.resize(self.create_input_vec(img, i), (1, self._input_dim))
